...
class AIService:
    ...
    def save_image(self, image_url: str, file_name: str):
        ...
        if response.status_code == 200:
            ...
            self.s3.upload_fileobj(
                BytesIO(response.content), self.BUCKET_NAME, file_name
            )
    ...
    def refine_prompt(self, state: State):
        ...
        response = self.llm.invoke(
            f"다음 문장을 이미지 생성용으로 개선하고, 개선한 문장만 출력해줘: {user_input}"
        )
    ...
    def generate_image(self, state: State) -> State:
        ...
        return {
            "prompt": prompt,
            "image_url": "error: openai server error after retries",
        }
